...
| `credential` | `TTL` (token lifetime, positive to 24h, default 1h) | 256-bit CSPRNG token · base64-URL no-padding encoding · HMAC-SHA256 hash with the library pepper · `purpose \|\| 0x00 \|\| subject \|\| 0x00 \|\| token` binding · constant-time compare run before the expiry check so the expiry step does not leak "token existed vs. did not exist"; the compare itself returns early on length mismatch, so wall-clock time can still reveal whether `storedHash` is the 64-hex-char shape (see `Verify` for the precise property) |
